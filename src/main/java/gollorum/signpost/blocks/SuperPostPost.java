...
public abstract class SuperPostPost extends BlockContainer {
    ...
    @Override
    public void onBlockClicked(World world, int x, int y, int z, EntityPlayer playerIn) {
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer playerIn, int facing, float hitX,
        ...
        if (isHitWaystone(hit)) {
            ...
            if (player.getHeldItem()
                ...
                .getItem() instanceof CalibratedPostWrench) {
                    if (!canUse(player, superTile)) {
                        return true;
                    }
                    rightClickCalibratedWrench(hit, superTile, player, x, y, z);
                } else if (player.getHeldItem()
                    .getItem() instanceof PostBrush) {
                        if (!canUse(player, superTile)) {
                            return true;
                        }
                        ...
                        if (superTile.getPaintObject() == null) {
                            ...
                            NetworkHandler.netWrap.sendTo(new RequestTextureMessage(x, y, z), player);
                        }
                    } else if (Block.getBlockFromItem(
                        player.getHeldItem()
                            .getItem()) instanceof BasePost) {
                                if (rightClickBase(superTile, player, x, y, z)) {
    ...
    protected boolean canUse(EntityPlayerMP player, SuperPostPostTile tile) {
        return ClientConfigStorage.INSTANCE.getSecurityLevelSignpost()
            .canUse(player, "" + tile.owner);
    }
            ...
            if (!PostHandler.doesPlayerKnowNativeWaystone(player, ws)) {
                if (!ClientConfigStorage.INSTANCE.deactivateTeleportation()) {
                    NetworkHandler.netWrap
                        .sendTo(new ChatMessage("signpost.discovered", "<Waystone>", ws.getName()), player);
                }
                PostHandler.addDiscovered(player.getUniqueID(), ws);
            }
        } else {
            if (!ClientConfigStorage.INSTANCE.deactivateTeleportation()
                && ClientConfigStorage.INSTANCE.getSecurityLevelWaystone()
                    .canUse(player, "" + ws.owner)) {
    ...
    private boolean rightClickBase(SuperPostPostTile superTile, EntityPlayerMP player, int x, int y, int z) {
        ...
        if (!(ClientConfigStorage.INSTANCE.getSecurityLevelSignpost()
            .canUse(player, "" + superTile.owner) && SPEventHandler.INSTANCE.checkWaystoneCount(player))) {
            return true;
        }
        ...
        MinecraftForge.EVENT_BUS.post(
            new UpdateWaystoneEvent(
                UpdateWaystoneEvent.WaystoneEventType.PLACED,
    ...
    private boolean preShiftClick(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y,
        int z) {
        if (isHitWaystone(hitObj)) {
            superTile.destroyWaystone();
            return false;
    ...
    public abstract void clickWrench(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y,
        int z);

    public abstract void rightClickWrench(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x,
        int y, int z);

    public abstract void shiftClickWrench(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x,
        int y, int z);

    public abstract void clickCalibratedWrench(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x,
        int y, int z);

    public abstract void rightClickCalibratedWrench(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player,
        int x, int y, int z);

    public abstract void shiftClickCalibratedWrench(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player,
        int x, int y, int z);

    public abstract void clickBrush(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y,
        int z);

    public abstract void rightClickBrush(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x,
        int y, int z);

    public abstract void click(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y, int z);

    public abstract void rightClick(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y,
        int z);

    public abstract void shiftClick(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y,
        int z);

    public abstract void clickBare(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y,
        int z);

    public abstract void shiftClickBare(Object hitObj, SuperPostPostTile superTile, EntityPlayerMP player, int x, int y,
        int z);
