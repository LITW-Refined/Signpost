...
public class BaseModelPost extends BlockContainer {
    ...
    public static final String[] allTypeNames = { "simple0", "simple1", "simple2", "detailed0", "detailed1", "aer",
        "dwarf", "ygnar" };
    public static final String[] allDefaultVillageTypeNames = { "simple0", "simple1", "simple2", "detailed0",
        "detailed1" };
    ...
    public static enum ModelType {
        ...
        private ModelType(int ID, String name) {
            this(
                ID,
                name,
                new ResourceLocation("signpost:models/block/" + name + ".obj"),
                new ResourceLocation("signpost:textures/blocks/waystone.png"));
        }
    ...
    public boolean onBlockActivated(World worldIn, int x, int y, int z, EntityPlayer playerIn, int side, float hitX,
        ...
        if (!worldIn.isRemote) {
            ...
            if (ws == null) {
                ws = new BaseInfo(
                    BasePost.generateName(),
                    new MyBlockPos(x, y, z, playerIn.dimension),
            ...
            if (!playerIn.isSneaking()) {
                if (!PostHandler.doesPlayerKnowNativeWaystone((EntityPlayerMP) playerIn, ws)) {
                    if (!ClientConfigStorage.INSTANCE.deactivateTeleportation()) {
                        NetworkHandler.netWrap.sendTo(
                            new ChatMessage("signpost.discovered", "<Waystone>", ws.getName()),
                ...
                if (!ClientConfigStorage.INSTANCE.deactivateTeleportation()
                    && ClientConfigStorage.INSTANCE.getSecurityLevelWaystone()
                        .canUse((EntityPlayerMP) playerIn, "" + ws.owner)) {
                    NetworkHandler.netWrap
                        .sendTo(new OpenGuiMessage(Signpost.GuiBaseID, x, y, z), (EntityPlayerMP) playerIn);
                }
            }
        }
        return true;
    ...
    public static void placeServer(World world, MyBlockPos blockPos, EntityPlayerMP player) {
        ...
        MinecraftForge.EVENT_BUS.post(
            new UpdateWaystoneEvent(
                ...
                name));
        NetworkHandler.netWrap
            .sendTo(new OpenGuiMessage(Signpost.GuiBaseID, blockPos.x, blockPos.y, blockPos.z), player);
    }
