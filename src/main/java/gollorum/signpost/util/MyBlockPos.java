...
public class MyBlockPos {
    ...
    public static int dim(World world) {
        ...
        } else return world.provider.dimensionId;
    }
    ...
    public Connection canConnectTo(BaseInfo inf) {
        ...
        if (ClientConfigStorage.INSTANCE.getMaxDist() > -1
            && distance(inf.teleportPosition) > ClientConfigStorage.INSTANCE.getMaxDist()) {
    ...
    public boolean equals(Object obj) {
        ...
        return other.x == this.x && other.y == this.y && other.z == this.z && sameDim(other);
    }
    ...
    public MyBlockPos front(EnumFacing facing, int i) {
        return this.withX(x -> x + facing.getFrontOffsetX() * i)
            .withY(y -> y + facing.getFrontOffsetY() * i)
            .withZ(z -> z + facing.getFrontOffsetZ() * i);
    }
