...
public class TextureHelper {
    ...
    public ResourceLocation getHeldBlockTexture(EntityPlayer player, World worldIn, int x, int y, int z) {
        try {
            ...
            Block block = Block.getBlockFromItem(
                player.getHeldItem()
                    .getItem());
            String textureName = block.getBlockTextureFromSide(side)
                .getIconName();
    ...
    public static int getFacingFromVector(float x, float y, float z) {
        ...
        for (EnumFacing enumfacing1 : EnumFacing.values()) {
            float f1 = x * (float) enumfacing1.getFrontOffsetX() + y * (float) enumfacing1.getFrontOffsetY()
                + z * (float) enumfacing1.getFrontOffsetZ();
    ...
    private static boolean endsWithIgnoreCase(String str1, String str2) {
        return str1.length() < str2.length() || str1.substring(str1.length() - str2.length())
