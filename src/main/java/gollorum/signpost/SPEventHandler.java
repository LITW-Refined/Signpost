...
public class SPEventHandler {
    ...
    public void oBlockPlace(PlaceEvent event) {
        ...
        if (event.block instanceof BasePost) {
            ...
            if (!(ClientConfigStorage.INSTANCE.getSecurityLevelWaystone()
                .canPlace(player) && checkWaystoneCount(player))) {
            ...
            if (!(ClientConfigStorage.INSTANCE.getSecurityLevelSignpost()
                .canPlace(player) && checkSignpostCount(player))) {
                tile.onBlockDestroy(blockPos);
                event.setCanceled(true);
            } else {
    ...
    public void onBlockBreak(BreakEvent event) {
        try {
            ...
            if (tile instanceof SuperPostPostTile && event.getPlayer()
                .getHeldItem() != null
                && (event.getPlayer()
                    .getHeldItem()
                    ...
                    || event.getPlayer()
                        .getHeldItem()
                        .getItem()
                        .equals(Items.snowball)
                    || event.getPlayer()
                        .getHeldItem()
                        .getItem()
                        .equals(Item.getItemFromBlock(Blocks.vine)))) {
                event.setCanceled(true);
                ((SuperPostPost) tile.blockType)
                    .onBlockClicked(event.world, event.x, event.y, event.z, event.getPlayer());
                return;
            }
            ...
            if (event.block instanceof BasePost) {
                ...
                if (!ClientConfigStorage.INSTANCE.getSecurityLevelWaystone()
                    .canUse(player, "" + t.getBaseInfo().owner)) {
                    event.setCanceled(true);
                } else {
