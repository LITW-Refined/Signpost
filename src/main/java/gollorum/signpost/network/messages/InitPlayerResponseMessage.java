...
public class InitPlayerResponseMessage implements IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        for (int i = 0; i < allowedCraftingModels.length; i++)
            allowedCraftingModels[i] = ByteBufUtils.readUTF8String(buf);
        allowedVillageModels = new String[buf.readInt()];
        for (int i = 0; i < allowedVillageModels.length; i++)
            allowedVillageModels[i] = ByteBufUtils.readUTF8String(buf);
    }
