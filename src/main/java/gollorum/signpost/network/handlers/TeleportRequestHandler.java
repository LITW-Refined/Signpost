...
public class TeleportRequestHandler implements IMessageHandler<TeleportRequestMessage, IMessage> {
            ...
            if (ClientConfigStorage.INSTANCE.skipTeleportConfirm()) {
                ...
                if (ClientConfigStorage.INSTANCE.getCost() != null) {
                    out = LanguageRegistry.instance()
                        .getStringLocalization("signpost.confirmTeleport");
                    if (out.equals("")) {
                        out = LanguageRegistry.instance()
                            .getStringLocalization("signpost.confirmTeleport", "en_US");
                    }
                    ...
                    out = LanguageRegistry.instance()
                        .getStringLocalization("signpost.confirmTeleportNoCost");
                    if (out.equals("")) {
                        out = LanguageRegistry.instance()
                            .getStringLocalization("signpost.confirmTeleportNoCost", "en_US");
                    }
                    out = out.replaceAll("<Waystone>", message.waystoneName);
                }
                FMLClientHandler.instance()
                    .getClient().thePlayer.addChatMessage(new ChatComponentText(out));
            }
        }
        return null;
    }
    ...
    public String getReplacement(String replace) {
        String ret = LanguageRegistry.instance()
            .getStringLocalization(replace);
        if (!ret.equals("")) {
            return ret;
        }
        if (!(ret = LanguageRegistry.instance()
            .getStringLocalization(replace, "en_US")).equals("")) {
            return ret;
        }
        return replace;
    }
