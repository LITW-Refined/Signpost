    ...
    public IMessage onMessage(TeleportMeMessage message, MessageContext ctx) {
        ...
        if (PostHandler.canTeleport(ctx.getServerHandler().playerEntity, message.base)) {
            ...
            if (world == null) {
                ...
                ServerConfigurationManager manager = FMLCommonHandler.instance()
                ...
                if (!(player.dimension == message.base.teleportPosition.dim)) {
                    WorldServer minecraftserver = MinecraftServer.getServer()
                        .worldServerForDimension(message.base.teleportPosition.dim);
                    manager.transferEntityToWorld(player, 1, (WorldServer) player.worldObj, minecraftserver);
                }
                ctx.getServerHandler().playerEntity.setPositionAndUpdate(
                    message.base.teleportPosition.x,
                    message.base.teleportPosition.y,
                    message.base.teleportPosition.z);
            }
        }
        return null;
    }
