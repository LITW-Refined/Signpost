...
public class BaseUpdateClientHandler implements IMessageHandler<BaseUpdateClientMessage, IMessage> {
    ...
    public IMessage onMessage(BaseUpdateClientMessage message, MessageContext ctx) {
        for (Entry<MyBlockPos, DoubleBaseInfo> now : PostHandler.getPosts()
            .entrySet()) {
            TileEntity tile = FMLClientHandler.instance()
                .getWorldClient()
                .getTileEntity(now.getKey().x, now.getKey().y, now.getKey().z);
            if (tile instanceof SuperPostPostTile) {
                ((SuperPostPostTile) tile).isWaystone = false;
            }
        }
        for (Entry<MyBlockPos, BigBaseInfo> now : PostHandler.getBigPosts()
            .entrySet()) {
            TileEntity tile = FMLClientHandler.instance()
                .getWorldClient()
                .getTileEntity(now.getKey().x, now.getKey().y, now.getKey().z);
        ...
        for (BaseInfo now : PostHandler.getNativeWaystones()) {
            TileEntity tile = FMLClientHandler.instance()
                .getWorldClient()
                .getTileEntity(now.blockPosition.x, now.blockPosition.y, now.blockPosition.z);
            if (tile instanceof SuperPostPostTile) {
        ...
        for (Entry<MyBlockPos, DoubleBaseInfo> now : PostHandler.getPosts()
            ...
            if (base != null && !(base.teleportPosition == null && base.owner == null)) {
                now.getValue().sign1.base = PostHandler.getAllWaystones()
                    .getByPos(base.blockPosition);
            }
            base = now.getValue().sign2.base;
            if (base != null && !(base.teleportPosition == null && base.owner == null)) {
                now.getValue().sign2.base = PostHandler.getAllWaystones()
                    .getByPos(base.blockPosition);
            }
        }
        for (Entry<MyBlockPos, BigBaseInfo> now : PostHandler.getBigPosts()
            ...
            if (base != null && !(base.teleportPosition == null && base.owner == null)) {
                now.getValue().sign.base = PostHandler.getAllWaystones()
                    .getByPos(base.blockPosition);
            }
            TileEntity tile = now.getKey()
                .getTile();
        }
        return null;
    }
