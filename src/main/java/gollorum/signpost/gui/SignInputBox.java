...
public class SignInputBox extends Gui {
    ...
    public void drawText() {
        ...
        if (isFocused && possible.size() > 0) {
            ...
            fontRenderer.drawString(
                possible.get(possibleIndex),
                (int) (this.x + width + 5),
                (int) (this.y + (scale * pHeight - fontRenderer.FONT_HEIGHT) / 2.0),
                Color.WHITE.getRGB());
        }
    }
    ...
    public int func_146197_a(int p_146197_1_, int p_146197_2_, boolean p_146197_3_) {
        ...
        for (int i1 = 0; i1 < l; ++i1) {
            if (flag1) {
                while (p_146197_3_ && k > 0
                    && this.getText()
                        .charAt(k - 1) == 32) {
                    --k;
                }

                while (k > 0 && this.getText()
                    .charAt(k - 1) != 32) {
                    --k;
                }
            } else {
    ...
    public void mouseClicked(int x, int y, int p_146192_3_) {
        isFocused = (x >= this.x && x < this.x + this.width && y >= this.y && y < this.y + this.height);
    ...
    public String correctTrim(int width) {
        int l = this.fontRenderer.trimStringToWidth(getText(), width)
            .length();
        if (getText().length() == l) {
            return getText();
        }
