...
class LibraryWaystoneHelper extends LibraryHelper {
            ...
            public boolean run() {
                ...
                try {
                    ...
                    if (emptySign != null) {
                        ...
                        final Sign sign = SuperPostPost.getSuperTile(pos)
                            .getEmptySigns()
                            .get(0);
                        if (pos.getTile() instanceof SuperPostPostTile
                            && ((SuperPostPostTile) pos.getTile()).isLoading()) {
                            return false;
                        } else {
    ...
    private Set<VillagePost> fetchOtherVillagesPosts() {
        ...
        for (Entry<MyBlockPos, Set<VillagePost>> entry : villagePosts.entrySet()) {
            if (!(entry.getKey()
                .equals(villageLocation) || containsMeAsTarget(entry.getValue()))) {
        ...
        for (VillagePost post : posts) {
            for (Sign sign : post.getSigns()) {
                if (sign != null && sign.isValid()
                    && sign.base != null
                    && sign.base.blockPosition != null
                    && sign.base.blockPosition.equals(waystoneLocation)) {
                    return true;
                }
            }
        }
    ...
    private class SignChooser {
            ...
            if (signs.size() > 0) {
                for (VillageSign sign : signs) {
                    ...
                    if (pos != null && SuperPostPost.getSuperTile(pos) != null) {
                        try {
                            SuperPostPost.getSuperTile(pos)
                                .getEmptySigns()
                                .get(0);
        ...
        private void fetchSigns(Set<VillagePost> posts) {
            for (VillagePost post : posts) {
                ...
                if (!signs.isEmpty()) {
                    this.signs
                        .add(new VillageSign(new VillagePost(post.getTopSignPosition(), post.desiredRotation), signs));
                }
            }
        }
        ...
        private void sortDistance() {
            signs.sort(
                (sign1, sign2) -> compareClosest(
                    sign1.post.getTopSignPosition(),
                    sign2.post.getTopSignPosition(),
                    villageLocation));
        }

        private void sortSignCount() {
            signs.sort((sign1, sign2) -> new Integer(sign2.signs.size()).compareTo(sign1.signs.size()));
        }
