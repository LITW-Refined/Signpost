...
public class ConfigHandler {
    ...
    public enum SecurityLevel {
        ...
        public static String[] allValues() {
            String[] ret = { ALL.toString(), OWNERS.toString(), CREATIVEONLY.toString(), OPONLY.toString() };
            return ret;
        }

        public boolean canPlace(EntityPlayerMP player) {
            return this.equals(ConfigHandler.SecurityLevel.ALL) || this.equals(OWNERS)
                || isOp(player)
        ...
        public boolean canUse(EntityPlayerMP player, String owner) {
            return this.equals(ConfigHandler.SecurityLevel.ALL) || isOp(player)
                || (this.equals(ConfigHandler.SecurityLevel.OWNERS) && owner.equals(
                    player.getUniqueID()
                        .toString()))
                || (isCreative(player) && this.equals(ConfigHandler.SecurityLevel.CREATIVEONLY));
        }
    ...
    private static void loadWorldGen() {
        ...
        disableVillageGeneration = config.getBoolean(
            "disableVillageGeneration",
            category,
            false,
            "Disables the generation of signposts and waystones in villages");

        villageMaxSignposts = config.getInt(
            "villageMaxSignposts",
            category,
            1,
            0,
        ...
        villageSignpostsWeight = config.getInt(
            "villageSignpostsWeight",
            category,
            20,
            0,
            Integer.MAX_VALUE,
        ...
        villageWaystonesWeight = config.getInt(
            "villageWaystoneWeight",
            category,
            20,
            0,
            Integer.MAX_VALUE,
        ...
        onlyVillageTargets = config.getBoolean(
            "onlyVillageDestinations",
            category,
            true,
    ...
    public static void loadClientSettings() {
        ...
        skipTeleportConfirm = config
            .getBoolean("skipTeleportConfirm", category, true, "Directly teleports the player on waystone right-click");
    ...
    public static void loadLimitation() {
        ...
        costBase = config.getInt(
            "constantPaymentPerTeleport",
            category,
            1,
            1,
            Integer.MAX_VALUE,
        ...
        signRec = RecipeCost.valueOf(
            config.getString(
                "signpostRecipeCost",
                category,
                "NORMAL",
                "Changes the recipe for signposts (NORMAL/EXPENSIVE/VERY_EXPENSIVE/DEACTIVATED)",
                RecipeCost.allValues()));

        waysRec = RecipeCost.valueOf(
            config.getString(
                "waystoneRecipeCost",
                category,
                "NORMAL",
                "Changes the recipe for waystones (NORMAL/EXPENSIVE/VERY_EXPENSIVE/DEACTIVATED)",
                RecipeCost.allValues()));

        allowedCraftingModels = config.getStringList(
            "waystoneModelCraftingTypes",
            category,
            BaseModelPost.allTypeNames,
            "Decide what waystone models can be crafted. You can look up the model names at https://www.curseforge.com/minecraft/mc-mods/signpost/pages/waystone-models",
            BaseModelPost.allTypeNames);

        allowedVillageModels = config.getStringList(
            "waystoneModelVillageTypes",
            category,
            BaseModelPost.allDefaultVillageTypeNames,
            "Decide what waystone models are generated in villages. You can look up the model names at https://www.curseforge.com/minecraft/mc-mods/signpost/pages/waystone-models",
            BaseModelPost.allTypeNames);
    ...
    public static void loadSecurity() {
        ...
        securityLevelWaystone = SecurityLevel.valueOf(
            config.getString(
                "waystonePermissionLevel",
                category,
                "ALL",
                "Defines which players can place and edit a waystone (ALL, OWNERS, CREATIVEONLY, OPONLY). OPs are always included, 'OWNERS' = everyone can place, only the owner+OPs can edit.",
                SecurityLevel.allValues()));

        securityLevelSignpost = SecurityLevel.valueOf(
            config.getString(
                "signpostPermissionLevel",
                category,
                "ALL",
                "Defines which players can place and edit a signpost (ALL, OWNERS, CREATIVEONLY, OPONLY). OPs are always included, 'OWNERS' = everyone can place, only the owner+OPs can edit.",
                SecurityLevel.allValues()));

        disableDiscovery = config.getBoolean(
            "disableDiscovery",
            category,
            false,
    ...
    public static String costName() {
        String out = LanguageRegistry.instance()
            .getStringLocalization(
                ClientConfigStorage.INSTANCE.getCost()
