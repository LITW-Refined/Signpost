...
public class PostHandler {
    ...
    public static void refreshBigPosts() {
        for (Entry<MyBlockPos, BigBaseInfo> now : bigPosts.entrySet()) {
            BigPostPostTile tile = (BigPostPostTile) now.getKey()
                .getTile();
            if (tile != null) {
                tile.isWaystone();
                tile.getBases();
     ...
     */
    public static boolean pay(EntityPlayer player, MyBlockPos origin, MyBlockPos destination) {
    ...
    public static boolean canPay(EntityPlayer player, MyBlockPos origin, MyBlockPos destination) {
    ...
    public static void doPay(EntityPlayer player, MyBlockPos origin, MyBlockPos destination) {
        if (ClientConfigStorage.INSTANCE.getCost() == null || ConfigHandler.isCreative(player)) {
            return;
        } else {
    ...
    public static int getStackSize(MyBlockPos origin, MyBlockPos destination) {
        return getStackSize((float) origin.distance(destination));
    }
    ...
    public static int getStackSize(float distance) {
        if (ClientConfigStorage.INSTANCE.getCostMult() == 0) {
            ...
            return (int) (distance / ClientConfigStorage.INSTANCE.getCostMult()
                + ClientConfigStorage.INSTANCE.getCostBase());
        }
    }
    ...
    public static void confirm(final EntityPlayerMP player) {
        ...
        SPEventHandler.scheduleTask(() -> {
            if (info == null) {
                ...
                doPay(player, new MyBlockPos(player), info.destination.teleportPosition);
                ...
                if (!(player.dimension == info.destination.teleportPosition.dim)) {
                    manager.transferPlayerToDimension(
                        player,
                        info.destination.teleportPosition.dim,
                        new SignTeleporter(info.world));
                }
                player.setPositionAndUpdate(
                    info.destination.teleportPosition.x + 0.5,
                    info.destination.teleportPosition.y + 1,
                    info.destination.teleportPosition.z + 0.5);
            }
        }, 1);
    }
        ...
        if (canTeleport(player, destination)) {
            ...
            if (world == null) {
                NetworkHandler.netWrap.sendTo(
                    new ChatMessage("signpost.errorWorld", "<world>", "" + destination.teleportPosition.dim),
                    player);
            } else {
                UUID uuid = player.getUniqueID();
                TeleportInformation teleportInformation = new TeleportInformation(
                    destination,
                    stackSize,
                    world,
                    new BoolRun() {
    ...
    public static boolean canTeleport(EntityPlayerMP player, BaseInfo target) {
        if (doesPlayerKnowWaystone(player, target)) {
            ...
            NetworkHandler.netWrap
                .sendTo(new ChatMessage("signpost.notDiscovered", "<Waystone>", target.getName()), player);
        }
        return false;
    }
    ...
    public static EntityPlayer getPlayerByName(String name) {
        for (Object player : MinecraftServer.getServer()
            ...
            if (player instanceof EntityPlayer && ((EntityPlayer) player).getCommandSenderName()
                .equals(name)) {
    ...
    private static class SignTeleporter extends Teleporter {
        ...
        @Override
        public void placeInPortal(Entity entityIn, double p_77185_2_, double p_77185_4_, double p_77185_6_,
            float p_77185_8_) {}

        @Override
        public boolean placeInExistingPortal(Entity entityIn, double p_77185_2_, double p_77185_4_, double p_77185_6_,
            float p_77185_8_) {
            return true;
        }
    ...
    public static Collection<String> getAllWaystoneNames() {
        Collection<String> ret = getAllWaystones().select(BaseInfo::getName);
